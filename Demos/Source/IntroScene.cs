...
namespace Satyrs_Playpark
{
    public class IntroScene : Spatial
    {
        ...
        const float cameraSlowPanSpeed = 0.1f;

        private void CleanPreviousScenes()
        {
            Node previousScene = GetTree().Root.GetChild(0);
        ...
        private Godot.Vector3 CalculatePosition(Godot.Vector3 circlePosition, float radiusCircle, float angle)
        {
        ...
        private void MoveCameraAroundPosition(Godot.Vector3 circlePosition, float radiusCircle, int step) {
            Godot.Vector3 circleCoord = CalculatePosition(circlePosition, radiusCircle, step / 1.0f);
            gameCamera.Translation = circleCoord;
        }   
        ...
        public void _on_IntroTimer_timeout()
        {
            ...
            if ( (introTimerCounter >= 10) && (introTimerCounter <= 310)) //Move the camera along an arc
            {
                //Godot.Vector3 cameraCirclePosition;
                MoveCameraAroundPosition(cameraCirclePosition, radiusCameraCircle, introTimerCounter - 10);
            }
            if (spritePlayer != null)
            {
                if (introTimerCounter <= 35)
