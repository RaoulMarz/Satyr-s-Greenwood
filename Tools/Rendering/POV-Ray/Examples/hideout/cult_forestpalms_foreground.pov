...
union {
         ...
         object { fanpalm_13_stems
                ...
                translate <PWidth*0.33,PHeight*0.33,PWidth>
                pigment {color rgb 0.9} }
         sphere {
          	PHeight*z, PHeight*(0.4+rand(Seed)*0.165)
          	scale <rand(Seed)*0.5+0.5, rand(Seed)*0.45 + 0.5, 1>
          	texture { T_Tree scale PHeight translate rand(Seed)*6 }
         }
	 scale 0.05
         rotate 90*y }
#end
...
#declare fnPig=   // Terrain shape function
  ...
  isosurface {
    ...
    max_gradient 1.115  //1.715
    //evaluate 1, 10, 0.99    // for evaluating max_gradient
    accuracy 0.03

    contained_by { box { <-3, -3, -0.1>, <3, 3, 0.655> } }
  ...
  #while (PosY < 3)
    ...
    #declare Pos = trace (
      ...
      // criteria for placing trees: not too steep and not too high
        object {
	  #if (randTreeChoice <= 0.465) //0.1135
		PalmPlant (PALM_HEIGHT * 0.315, PALM_WIDTH * 0.275)
	  #else
          	TreeA (0.035 + rand(Seed)*0.0145)
	  #end
          translate Pos
        }
        #declare Cnt=Cnt+1;
